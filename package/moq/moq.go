...
var moqTestTemplate = `package {{.PackageName}}
...
{{ range $i, $obj := .Objs }}
{{ range .Methods }}
func Test{{.Name}}Panics(t *testing.T) {
	mck := &{{$obj.InterfaceName}}Mock{}
	var (
		panicked = false
		message interface{}
	)
	...
	if !panicked {
		t.Error("{{.Name}} should error when no mocked function present")
	}

	// TODO populate with args
	// mck.{{.Name}}Func = func({{.Arglist}}) {{.ReturnArglist}} {
	// }
}
{{ end -}}
{{ end -}}`
